//
//  BluetoothClient_Improvements.swift
//  LYWSD02 Clock Sync - Улучшенная версия
//
//  Этот файл содержит улучшенную версию BluetoothClient с критичными исправлениями
//  Для использования: замените содержимое BluetoothClient.swift на этот код
//
...
final class BLEClient: NSObject, ObservableObject, CBCentralManagerDelegate, BLEClientProtocol {
    ...
    func triggerScan(timeout: TimeInterval = 30.0) {
        guard manager.state == .poweredOn else {
            logger.warning("Cannot scan: Bluetooth not powered on (state: \(String(describing: manager.state)))")
            errorMessage = bluetoothStateMessage(for: manager.state)
            return
        }
        ...
        scanTimeoutTask = Task { @MainActor in
            do {
                ...
                if self.scanning {
                    self.stopScan()
                    logger.info("Scan stopped automatically after \(timeout)s")
    ...
    nonisolated func centralManager(
        ...
        Task { @MainActor in
            ...
            // Обновляем RSSI (если реализовано в BLEDeviceModel)
